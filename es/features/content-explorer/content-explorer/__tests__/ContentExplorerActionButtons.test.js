...
describe('features/content-explorer/content-explorer/ContentExplorerActionButtons', function () {
  ...
  describe('onChooseClick', function () {
    ...
    test('should not call onChooseClick in MULTI_SELECT mode if selection is empty', function () {
      var onChooseClickSpy = sandbox.spy();
      var wrapper = renderComponent({
        contentExplorerMode: ContentExplorerModes.MULTI_SELECT,
        onChooseClick: onChooseClickSpy,
    ...
    test('should call onChooseClick in MULTI_SELECT mode if selection is empty and isNoSelectionAllowed is true', function () {
      var onChooseClickSpy = sandbox.spy();
      var wrapper = renderComponent({
        contentExplorerMode: ContentExplorerModes.MULTI_SELECT,
        onChooseClick: onChooseClickSpy,
  ...
  describe('renderStatus', function () {
    ...
    test('should show default status message for multi select', function () {
      var wrapper = renderComponent({
        contentExplorerMode: ContentExplorerModes.MULTI_SELECT,
        selectedItems: selectedItems
      });
      var statusMessageId = wrapper.find('.status-message').find('FormattedMessage').prop('id');
      expect(statusMessageId).toBe('boxui.contentExplorer.numSelected');
    });
    test('should show status message for multi select when items are selected and we can include subfolders and can select all', function () {
      var wrapper = renderComponent({
    ...
    test('should show status message for multi select when items are selected and we can include subfolders and cannot select all', function () {
      var wrapper = renderComponent({
        canIncludeSubfolders: true,
        contentExplorerMode: ContentExplorerModes.MULTI_SELECT,
    ...
    test('should show status element as PlainButton for multi select when onViewSelectedClick is provided', function () {
      var wrapper = renderComponent({
        contentExplorerMode: ContentExplorerModes.MULTI_SELECT,
        selectedItems: selectedItems,
    ...
    test('should show status element as Button when onSelectedClick is provided but onViewSelectedClick is not', function () {
      var wrapper = renderComponent({
        contentExplorerMode: ContentExplorerModes.MULTI_SELECT,
        onSelectedClick: jest.fn(),
        selectedItems: selectedItems
      });
      expect(wrapper.find('Button').at(0).prop('className')).toBe('status-message');
    });
    test('should show status element as span when onSelectedClick is not provided nor is onViewSelectedClick', function () {
      var wrapper = renderComponent({
        contentExplorerMode: ContentExplorerModes.MULTI_SELECT,
        selectedItems: selectedItems
      });
