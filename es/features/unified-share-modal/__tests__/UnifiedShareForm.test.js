...
describe('features/unified-share-modal/UnifiedShareForm', function () {
  ...
  var defaultUserContacts = defaultContacts.filter(function (_ref) {
    var type = _ref.type;
    return type === 'user';
  });
  var testPermission = {
  ...
  describe('render()', function () {
    ...
    test('should render a default component with ACI toggle if enabled ', function () {
      var wrapper = getWrapper({
        onAdvancedContentInsightsToggle: jest.fn()
      });
      expect(wrapper.exists('AdvancedContentInsightsToggle')).toBe(true);
    });
    test('should not render a default component with ACI toggle if disabled ', function () {
      var wrapper = getWrapper();
  ...
  describe('handleSendSharedLink()', function () {
    test('should call onSendClick  and sendSharedLink with the correct params',
    /*#__PURE__*/
    _asyncToGenerator(
    /*#__PURE__*/
    regeneratorRuntime.mark(function _callee3() {
      var data, onSendClick, sendSharedLink, sharedLink, trackingProps, expectedParams, wrapper;
        ...
        while (1) {
          switch (_context3.prev = _context3.next) {
            case 0:
              ...
              expectedParams = _objectSpread({}, data, {
                numsOfRecipients: 2,
                numOfRecipientGroups: 2,
                permissionLevel: CAN_EDIT
              });
              wrapper = getWrapper({
                sendSharedLink: sendSharedLink,
                sharedLink: sharedLink,
                trackingProps: trackingProps
              });
              _context3.next = 9;
              return wrapper.instance().handleSendSharedLink(data);
  ...
  describe('componentDidUpdate()', function () {
    test.each(_templateObject(), false, [], [], 'collab restrictions do not change', false, defaultUserContacts.map(function (_ref5) {
      var email = _ref5.email;
      return email;
    }), [], 'collab restrictions change but justification is not allowed', true, ['unmatched_email@example.com'], [], 'collab restrictions change but no restricted collabs are present')('should not fetch justification reasons when $conditionDescription', function (_ref6) {
      var isCollabRestrictionJustificationAllowed = _ref6.isCollabRestrictionJustificationAllowed,
          restrictedCollabEmails = _ref6.restrictedCollabEmails,
          restrictedGroups = _ref6.restrictedGroups;
      ...
      wrapper.setProps({
        ...
        restrictedCollabEmails: restrictedCollabEmails,
        restrictedGroups: restrictedGroups
      });
      expect(getJustificationReasons).toHaveBeenCalledTimes(0);
    });
    test.each(_templateObject2(), true, defaultUserContacts.map(function (_ref7) {
      var email = _ref7.email;
      return email;
    }), [], 'collab restrictions change, justification is allowed and restricted collabs are present', true, [], [12345], 'collab group restrictions change, justification is allowed and restricted collabs are present')('should fetch justification reasons when $conditionDescription', function (_ref8) {
      var isCollabRestrictionJustificationAllowed = _ref8.isCollabRestrictionJustificationAllowed,
          restrictedCollabEmails = _ref8.restrictedCollabEmails,
          restrictedGroups = _ref8.restrictedGroups;
      var getJustificationReasons = jest.fn().mockResolvedValue({});
      var wrapper = getWrapper({
        getJustificationReasons: getJustificationReasons,
        isCollabRestrictionJustificationAllowed: false,
  ...
  describe('fetchJustificationReasons()', function () {
    ...
    var justificationReasonOptions = justificationReasons.map(function (_ref9) {
      var id = _ref9.id,
          title = _ref9.title;
      return {
        value: id,
    ...
    test('should fetch justification reasons, pass them on to collab email form and store classification label id',
    /*#__PURE__*/
    _asyncToGenerator(
    /*#__PURE__*/
    regeneratorRuntime.mark(function _callee4() {
      var classificationLabelId, getJustificationReasons, wrapper, collabEmailForm;
      return regeneratorRuntime.wrap(function _callee4$(_context4) {
        while (1) {
          switch (_context4.prev = _context4.next) {
            ...
            case 5:
              collabEmailForm = wrapper.find('[data-testid="invite-collaborator-container"] EmailForm');
  ...
  describe('updateInviteCollabsContacts()', function () {
    test('should store updated contacts and call setUpdatedContacts', function () {
      ...
      wrapper.instance().updateInviteCollabsContacts(defaultContacts);
      var collabEmailForm = wrapper.find('[data-testid="invite-collaborator-container"] EmailForm');
      expect(collabEmailForm).toHaveLength(1);
    ...
    test('should call onRemoveAllRestrictedCollabs when update results in all restricted contacts being removed', function () {
      var onRemoveAllRestrictedCollabs = jest.fn();
      var restrictedCollabEmails = ['y@example.com', 'z@example.com'];
      var contactsMinusOneRestriction = defaultContacts.filter(function (value) {
        return value !== 'y@example.com';
      });
      var contactsMinusAllRestrictions = defaultContacts.filter(function (value) {
        return restrictedCollabEmails.includes(value);
      });
      var wrapper = getWrapper({
        onRemoveAllRestrictedCollabs: onRemoveAllRestrictedCollabs,
        restrictedCollabEmails: restrictedCollabEmails
      }); // Adding new contacts, no call expected

      wrapper.instance().updateInviteCollabsContacts(defaultContacts);
      expect(onRemoveAllRestrictedCollabs).toHaveBeenCalledTimes(0); // Minus one restricted contact

      wrapper.instance().updateInviteCollabsContacts(contactsMinusOneRestriction);
      expect(onRemoveAllRestrictedCollabs).toHaveBeenCalledTimes(0); // Minus all restricted contacts
    ...
    test('should call onRemoveAllRestrictedCollabs when update results in all restricted contacts including groups being removed', function () {
      var onRemoveAllRestrictedCollabs = jest.fn();
      var restrictedCollabEmails = ['y@example.com'];
      var restrictedGroups = ['12345'];
      var contactsMinusOneRestriction = defaultContacts.filter(function (value) {
        return value !== 'y@example.com';
      });
      var contactsMinusAllRestrictions = defaultContacts.filter(function (id, value) {
        return restrictedGroups.includes(id) || restrictedCollabEmails.includes(value);
      });
      var wrapper = getWrapper({
        onRemoveAllRestrictedCollabs: onRemoveAllRestrictedCollabs,
        restrictedCollabEmails: restrictedCollabEmails
      ...
      wrapper.instance().updateInviteCollabsContacts(defaultContacts);
      expect(onRemoveAllRestrictedCollabs).toHaveBeenCalledTimes(0); // Minus one restricted contact, group still remains

      wrapper.instance().updateInviteCollabsContacts(contactsMinusOneRestriction);
      expect(onRemoveAllRestrictedCollabs).toHaveBeenCalledTimes(0); // Minus all restricted contacts, including the group

      wrapper.instance().updateInviteCollabsContacts(contactsMinusAllRestrictions);
      expect(onRemoveAllRestrictedCollabs).toHaveBeenCalledTimes(1);
    });
  });
  describe('shouldRequireCollabJustification()', function () {
    test.each(_templateObject3(), false, [], [], false, true, [], [], false, true, [defaultUserContacts[0].value], [], true, true, defaultUserContacts.map(function (_ref12) {
      var value = _ref12.value;
      return value;
    }), [], true, true, [], [12345], true)('should return $expectedResult when isCollabRestrictionJustificationAllowed is $isCollabRestrictionJustificationAllowed, restrictedCollabEmails is $restrictedCollabEmails and restrictedGroups is $restrictedGroups', function (_ref13) {
      var isCollabRestrictionJustificationAllowed = _ref13.isCollabRestrictionJustificationAllowed,
          restrictedCollabEmails = _ref13.restrictedCollabEmails,
          restrictedGroups = _ref13.restrictedGroups,
          expectedResult = _ref13.expectedResult;
      var wrapper = getWrapper({
        isCollabRestrictionJustificationAllowed: isCollabRestrictionJustificationAllowed,
        item: defaultItem,
        restrictedCollabEmails: restrictedCollabEmails,
