...
describe('features/unified-share-modal/SharedLinkSection', function () {
  ...
  test('should match snapshot when shared link expiration is set', function () {
    var wrapper = getWrapper({
      sharedLink: {
        accessLevel: 'peopleInYourCompany',
        canChangeAccessLevel: true,
  ...
  test('should have aria-label and role on shared link expiration badge when expiration timestamp exists', function () {
    ...
    var toggle = wrapper.find('Toggle'); // The aria-label attr is on a span, which is contained within the label of the Toggle component

    var spanLabel = toggle.dive().find('.shared-link-expiration-badge');
  ...
  describe('componentDidUpdate()', function () {
    ...
    test('should call onSharedLinkCopy with current permission level when copy button is clicked', function () {
      var onSharedLinkCopy = jest.fn();
      var sharedLink = {
        url: 'http://example.com/',
        isNewSharedLink: false,
        permissionLevel: CAN_EDIT
      ...
      var wrapper = getWrapper({
        sharedLink: sharedLink,
        trackingProps: {
          onSharedLinkCopy: onSharedLinkCopy
        }
      });
