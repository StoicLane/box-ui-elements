...
describe('elements/content-sidebar/ActivityFeed/annotations/AnnotationActivity', function () {
  ...
  test('should not render annotation activity menu when can_delete is false and can_edit is false and can_resolve is false', function () {
  ...
  test.each(_templateObject(), false, false, true, true, false, false, false, true, false, false, true, true, true, true, false, true, false, true, true, true, true)('should correctly render annotation activity when canDelete: $canDelete and canEdit: $canEdit and canResolve: $canResolve', function (_ref) {
    ...
    expect(wrapper.find('ForwardRef(withFeatureConsumer(ActivityMessage))').prop('tagged_message')).toEqual(mockActivity.item.description.message);
  });
  ...
  test('should not render file version link if hasVersions is false', function () {
    var wrapper = getWrapper({
      hasVersions: false
    });
