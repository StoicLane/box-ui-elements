...
function (_Component) {
  ...
  function ContentPicker(props) {
    ...
    _defineProperty(_assertThisInitialized(_this), "createFolderCallback", function (name) {
      ...
      _this.api.getFolderAPI().create(id, name.trim(), function () {
        _this.fetchFolder(id);
      }, function (_ref) {
    ...
    _defineProperty(_assertThisInitialized(_this), "select", function (item) {
      ...
      var existInSelected = selectedKeys.indexOf(cacheKey) !== -1; // Existing object could have mutated and we just need to update the
    ...
    function () {
      ...
      regeneratorRuntime.mark(function _callee(item) {
        var selected, id, type, cacheKey, updatedItem;
        return regeneratorRuntime.wrap(function _callee$(_context) {
          while (1) {
            switch (_context.prev = _context.next) {
              case 0:
                selected = _this.state.selected;
                id = item.id, type = item.type; // $FlowFixMe

                cacheKey = _this.api.getAPI(type).getCacheKey(id);
