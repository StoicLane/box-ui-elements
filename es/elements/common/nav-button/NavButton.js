...
var NavButton = React.forwardRef(function (props, ref) {
  ...
  var match = useMatch(path);
  var isExactMatch = !exact || location.pathname === path;
  var isActiveValue = isActive ? isActive(match, location) : match && isExactMatch;
  ...
  var handleClick = function handleClick(event) {
    ...
    if (!event.defaultPrevented && isLeftClick(event)) {
      ...
      navigate(_typeof(to) === 'object' ? to.pathname : to, {
        state: _typeof(to) === 'object' ? to.state : {},
        replace: replace
      });
    }
  };
