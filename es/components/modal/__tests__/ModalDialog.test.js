...
describe('components/modal/ModalDialog', function () {
  ...
  test('render back button when onRequestBack is defined', function () {
    expect(wrapper.find('.modal-back-button').length).toBeTruthy();
  });
  test('should not render back button if onRequestBack is null', function () {
    wrapper.setProps({
      onRequestBack: null
    });
    expect(wrapper.find('.modal-back-button').length).toBeFalsy();
  });
  test('should call onRequestBack when back button is clicked on', function () {
