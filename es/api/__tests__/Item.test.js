...
describe('api/Item', function () {
  ...
  describe('validateSharedLinkRequest()', function () {
    ...
    test.each(_templateObject(), undefined, MOCK_SUFFICIENT_PERMISSIONS, 'itemID is missing', MOCK_ITEM_ID, undefined, 'itemPermissions is missing')('should throw a bad item error if $description', function (_ref) {
      ...
      expect(function () {
        return item.validateSharedLinkRequest(itemID, itemPermissions);
    ...
    test.each(_templateObject2(), {
      ...
      expect(function () {
        return item.validateSharedLinkRequest(MOCK_ITEM_ID, itemPermissions);
      }).toThrowError(MOCK_PERMISSIONS_ERROR);
      expect(item.errorCode).toBe(ERROR_CODE_SHARE_ITEM);
    });
    test('should not throw an error if the request is valid', function () {
      expect(function () {
        return item.validateSharedLinkRequest(MOCK_ITEM_ID, MOCK_SUFFICIENT_PERMISSIONS);
      }).not.toThrow();
      expect(item.errorCode).toBeUndefined();
    });
    test('should skip `can_set_share_access` check when canSkipSetShareAccessPermission is true', function () {
      var itemPermissions = {
        can_share: true,
        can_set_share_access: false
      };
      expect(function () {
        return item.validateSharedLinkRequest(MOCK_ITEM_ID, itemPermissions, true);
      }).not.toThrow();
      expect(item.errorCode).toBeUndefined();
    });
    test('should not skip `can_set_share_access` check when canSkipSetShareAccessPermission is false', function () {
      var itemPermissions = {
        can_share: true,
        can_set_share_access: false
      };
      expect(function () {
        return item.validateSharedLinkRequest(MOCK_ITEM_ID, itemPermissions, false);
      }).toThrowError(MOCK_PERMISSIONS_ERROR);
      expect(item.errorCode).toBe(ERROR_CODE_SHARE_ITEM);
    });
  });
  describe('share()', function () {
    ...
    var errorHandlerSpy;
    var getUrlSpy;
    var validateSharedLinkRequestSpy;
    var MOCK_DATA = {
      shared_link: '',
      permissions: {}
    };
    ...
    describe('with missing data', function () {
      ...
      beforeEach(function () {
        validateSharedLinkRequestSpy.mockImplementation(function () {
